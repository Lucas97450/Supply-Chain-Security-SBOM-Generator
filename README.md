...
### 7) Scan CVE avec Grype

- **Lien SBOM ↔ CVE**: Grype lit le SBOM et retourne les vulnérabilités.
- **Seuil CVSS**: appliqué via policy (fail si CVSS ≥ 7).
- **Sortie**: JSON pour consommation CI.

```bash
grype sbom:sbom.json -o json > grype-report.json
```

### 8) Policy-as-Code (OPA / Conftest)

- **Règle Rego**: échoue si `cvss_score >= 7`.
- **Où**: `security/policies/security.rego`.
- **Tester**:
...
deny contains msg if {
  ...
  vuln.cvss_score >= 7
  msg := sprintf("%s : CVSS %2.1f", [vuln.id, vuln.cvss_score])
}
```
